...
function addproblem($title, $Cvalue, $time_limit, $memory_limit, $description, $input, $output, $sample_input, $sample_output, $hint, $source, $spj,$OJ_DATA) {

//	$spj=($spj);

	$sql = "INSERT into `problem` (`title`,`category`,`time_limit`,`memory_limit`,
        `description`,`input`,`output`,`sample_input`,`sample_output`,`hint`,`source`,`spj`,`in_date`,`defunct`)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,NOW(),'Y')";
	//echo $sql;

	$pid =pdo_query( $sql,$title, $Cvalue,$time_limit,$memory_limit,$description,$input,$output,
